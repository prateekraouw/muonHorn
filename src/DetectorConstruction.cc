...
DetectorConstruction::DetectorConstruction(const HornParams& hp)
...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
    ...
    // Horn axial extents (same length L for both horns; horn2 shifted by spacing)
    const G4double z1Beg = hp_.zMin_mm * mm;
    const G4double z1End = hp_.zMax_mm * mm;
    const G4double L     = z1End - z1Beg;                     // horn length
    const G4double z2Beg = (hp_.zMin_mm + hp_.spacing_mm) * mm;
    const G4double z2End = z2Beg + L;
    
    // Radii
    const G4double r_neck  = hp_.r_neck_mm * mm;
    const G4double Rout    = hp_.Rout_mm   * mm;
    
    // Parabola: r^2(z) = r_neck^2 + a*(z - z1Beg)  ⇒ at z1End:
    const G4double a_mm    = hp_.a_mm;               // (mm)
    const G4double r_max   = hp_.r_max_mm * mm;
    
    // Shell thicknesses (keep your values)
    const G4double tInner  = 1.0*mm;
    const G4double tOuter  = 1.0*mm;
    ...
    const G4double I1 = +hp_.I_A;  // Focus
    const G4double I2 = -hp_.I_A;  // Defocus
    ...
    HornField* hf2 = new HornField(I2, /*zt_mm=*/z2End, a_mm, r_neck, Rout, z2Beg, z2End, z2Beg, z2End, -1);
    ...
    fmHorn1->SetDeltaOneStep(0.05*mm);           // Increased from 0.05*mm
    fmHorn1->SetMinimumEpsilonStep(1e-4);       // Relaxed from 1e-4
    fmHorn1->SetMaximumEpsilonStep(1e-3);       // Increased from 1e-3
    fmHorn1->GetChordFinder()->SetDeltaChord(0.5*mm);  // Increased from 0.5*mm
    ...
    fmHorn2->SetDeltaOneStep(0.05*mm);
    fmHorn2->SetMinimumEpsilonStep(1e-4);
    fmHorn2->SetMaximumEpsilonStep(1e-3);
    fmHorn2->GetChordFinder()->SetDeltaChord(0.5*mm);
    fmHorn2->SetDeltaIntersection(0.1*mm);      // Intersection accuracy
    fmHorn2->SetAccuraciesWithDeltaOneStep(0.5*mm); // Consistent accuracy
    ...
    // ===================== HORN 1 GEOMETRY =====================
    {
        ...
        new G4PVPlacement(rotation, G4ThreeVector(0,0,-hHalfZ+25*cm),
    ...
    // Detector 1
    G4double detector1_position = z1End+100*mm;
    ...
    // Detector 2
    G4double detector2_position = z2End+100*mm;
